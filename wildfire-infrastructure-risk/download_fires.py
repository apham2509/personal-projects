"""Download historical VIIRS fire detections for a region, year by year.

The FIRMS area API returns at most 5 days per request, so each year is
fetched in 5-day chunks and written to
data/fires/<region>/fires_<SOURCE>_<YEAR>.csv.gz.
...
Usage:
    python download_fires.py                                   # iberia, 2018-2025
    python download_fires.py --region greece --start 2022 --end 2022
    python download_fires.py --region world --start 2024 --end 2024   # large!
"""
...
def main() -> int:
    ...
    parser.add_argument("--outdir", default=None, help="Default: data/fires/<region>")
    ...
    outdir = Path(args.outdir) if args.outdir else Path("data/fires") / region.slug
    outdir.mkdir(parents=True, exist_ok=True)
    print(f"Region: {region.slug} (bbox {region.bbox}), source {args.source}")
    ...
    for year in range(args.start, args.end + 1):
        ...
        if out_file.exists() and not args.force:
            ...
            continue
        t0 = time.time()
        df = download_year(year, args.source, region.bbox, args.pause)
