...
- **Ingestion** works in 5-day windows (the API maximum), with quota-aware retry: world-scale responses cost ~200 of the 5,000-per-10-min transaction budget each, so the client blocks and resumes rather than failing. Network errors and temporary HTTP errors retry three times with short backoff and a 10-second connection timeout, separate from the longer download timeout. If those attempts fail, the scheduled workflow retries the full build once on a macOS runner, using a different hosting network from Ubuntu. Only complete live builds deploy; if both runners cannot reach FIRMS, the workflow fails and the last successful Pages deployment stays online. Permanent API errors and code failures do not trigger the fallback. Chunks are date-disjoint by construction and deduplicated defensively, and each daily build re-derives the NRT window from scratch, so reruns are idempotent.
